...
// 활성화된 Pro가 팝업 UI에서만 보이고 저장·유지되지 않는 회귀를 막는다.
describe("popup: 라이선스 활성화 상태 유지(회귀)", () => {
  ...
  test("활성화 성공 후 팝업을 다시 열면(새 컨텍스트) Pro 배지와 라이선스 정보가 복원된다", async () => {
    const env = createPopupEnv({
      fetchImpl: () =>
        Promise.resolve({ status: 200, json: async () => ({ ok: true, sessionToken: "sess-token-abc", entitlement: proEntitlement() }) }),
    });
    env.loadPopupScript({ buildConfig: devBuild });
    await wait(150);
    ...
    env.click(env.document.getElementById("cloakli-activate-license-btn"));
    await wait(120);
    assert.match(env.document.getElementById("cloakli-license-message").textContent, /Pro가 활성화되었습니다/);

    // 팝업 재오픈: 이전 팝업의 chrome.storage 상태로 완전히 새로운 팝업 환경을 만든다.
    const reopened = createPopupEnv({
      chrome: { initialStorage: env.chrome.__storageData },
      fetchImpl: () => Promise.reject(new TypeError("팝업 재오픈은 네트워크 없이도 Pro를 복원해야 한다")),
    });
    reopened.loadPopupScript({ buildConfig: devBuild });
    await wait(60);
    ...
    assert.equal(reopened.document.getElementById("cloakli-license-free-actions").hidden, true);
    assert.equal(reopened.document.getElementById("cloakli-license-masked-key").textContent, "•••• 1234");
  });

  test("storage 저장이 실패하면 성공 메시지를 표시하지 않고 실패 안내를 보여준다", async () => {
    const env = createPopupEnv({
      fetchImpl: () =>
        Promise.resolve({ status: 200, json: async () => ({ ok: true, sessionToken: "sess-token-abc", entitlement: proEntitlement() }) }),
    });
    env.loadPopupScript({ buildConfig: devBuild });
    await wait(150);
    env.chrome.__storageSetFails = true; // 이후 모든 저장 실패
    ...
    const message = env.document.getElementById("cloakli-license-message").textContent;
    assert.ok(!/Pro가 활성화되었습니다/.test(message), "저장 실패 시 성공 메시지를 표시하면 안 된다: " + message);
    assert.match(message, /활성화 상태를 저장하지 못했습니다/);
    assert.equal(env.document.getElementById("cloakli-license-active-info").hidden, true, "License Pro UI로 전환되면 안 된다");
  });

  test("서버가 비활성 라이선스를 거부하면(license_expired) 성공 메시지 없이 사유를 보여준다", async () => {
    const env = createPopupEnv({
      fetchImpl: () => Promise.resolve({ status: 400, json: async () => ({ ok: false, error: "license_expired" }) }),
    });
    env.loadPopupScript({ buildConfig: devBuild });
    await wait(150);
    ...
    assert.match(env.document.getElementById("cloakli-license-message").textContent, /만료된 라이선스/);
    assert.ok(!env.chrome.__storageData.cloakliLicenseSession, "거부된 활성화가 세션을 남기면 안 된다");
    assert.ok(!env.chrome.__storageData["cloakli.entitlement.v1"], "거부된 활성화가 통합 레코드를 남기면 안 된다");
  });
});
