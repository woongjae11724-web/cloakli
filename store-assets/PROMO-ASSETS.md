# 프로모션 이미지 제작 지침 (아직 미제작 — 정직 표기)

> 현재 저장소에는 실제 프로모션 이미지가 없다. 임시/빈 이미지를 제출용으로 만들지 않는다.
> 대시보드가 실제로 요구하는 규격은 업로드 화면에 표시되는 값을 기준으로 한다 — 아래는
> 일반적으로 알려진 규격이며, 화면과 다르면 화면이 우선이다.
